...
#                hyp.append(backLayer(2))
    ...
    def forward(self, x):
        ...
        for i in range(self.n):
            y, nlJ = self.hyp[i](10*y) #Make it skinny
            y = y/10
            lJ = nlJ + lJ
...
if __name__ == "__main__":
    trainingCurve = open('deepAdversarialWeights/curve', 'w')
...
#        loss = L(yu, target)#/batchsize # + torch.sum(ld)
        ...
        if epoch%100 == 0:
            torch.save(model, 'deepAdversarialWeights/epoch' + str(epoch))
            trainingCurve = open('deepAdversarialWeights/curve', 'a')
