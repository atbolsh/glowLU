...
model = flowGAN(6).cuda()
optimizer = optim.Adam(model.parameters(), lr=1e-3)

if __name__ == "__main__":
    trainingCurve = open('glowAdvBatchnorm/curve', 'w')
...
#        print(model.aff[0].b.weight)
        ...
        if epoch%100 == 0:
            torch.save(model, 'glowAdvBatchnorm/epoch' + str(epoch))
            trainingCurve = open('glowAdvBatchnorm/curve', 'a')
