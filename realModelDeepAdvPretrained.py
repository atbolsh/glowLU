...
model = torch.load('deepWeights/epoch229900')
optimizer = optim.Adam(model.parameters(), lr=1e-4)

if __name__ == "__main__":
    trainingCurve = open('deepWeightsPretrained/curve', 'w')
...
#        loss = L(yu, target)#/batchsize # + torch.sum(ld)
        ...
        if epoch%100 == 0:
            torch.save(model, 'deepWeightsPretrained/epoch' + str(epoch))
            trainingCurve = open('deepWeightsPretrained/curve', 'a')
