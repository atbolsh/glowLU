...
#        print(m1.size())
#        print(y[:, 1].size()) 
        y1 = torch.exp(m1 - torch.sum(m1)/s)*x[:, 1] + b1
...
#        x0 = (z[:, 0] - b2)/torch.exp(m2)
        x0 = (y[:, 0] - b2)/torch.exp(m2 - torch.sum(m2)/s)
        ...
        x1 = (y[:, 1] - b1)/torch.exp(m1 - torch.sum(m1)/s)

        return torch.stack((x0, x1)).t(), m1 + m2 - torch.sum(m1)/s - torch.sum(m2)/s
