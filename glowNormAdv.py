...
class flowGAN(nn.Module):
    def __init__(self, n):
        ...
        for i in range(n):
            if i % 2 == 0:
                aff.append(affine2(2, hidden=1024))
            else:
                aff.append(affine2(2, hidden=1024))
...
if __name__ == "__main__":
    trainingCurve = open('glowNormAdv/curve', 'w')
...
#        print(model.aff[0].b.weight)
        ...
        if epoch%100 == 0:
            torch.save(model, 'glowNormAdv/epoch' + str(epoch))
            trainingCurve = open('glowNormAdv/curve', 'a')
